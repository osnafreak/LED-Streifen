...
#define MIRR_V 1			// отразить текст по вертикали (0 / 1)
...
uint8_t getFont(uint8_t font, uint8_t row)
{
	...
	if (font <= 90)								// для английских букв и символов
...
void drawLetter(uint8_t index, uint8_t letter, int16_t offset, uint32_t color)
{
	...
	MColor letterColor;
	if (color == 1) letterColor = mHSVfast(byte(offset * 10), 255, 255);
	else if (color == 2) letterColor = mHSV(byte(index * 30), 255, 255);
	else letterColor = mRGB(RGB24toR(color), RGB24toG(color), RGB24toB(color));
	...
	for (byte i = start_pos; i < finish_pos; i++)
	{
		...
		for (byte j = 0; j < LET_HEIGHT; j++)
		{
			...
			if (TEXT_DIRECTION) {
				fastLED.set(offset + i, TEXT_HEIGHT + j, thisBit ? letterColor : COLORS::mBlack);
				//if (thisBit) leds[getPixelNumber(offset + i, TEXT_HEIGHT + j)] = letterColor;
				//else drawPixelXY(offset + i, TEXT_HEIGHT + j, 0x000000);
			}
			else {
				fastLED.set(i, offset + TEXT_HEIGHT + j, thisBit ? letterColor : COLORS::mBlack);
	...
	{	// || (!BTcontrol && !gamemodeFlag)
		...
		while (text[i] != '\0') {
			if ((byte)text[i] > 191) {    // работаем с русскими буквами!
				...
				drawLetter(j, text[i], textOffset - j * (LET_WIDTH + SPACE), color);
				i++;
				j++;
			}
		}
		...
		//textOffset--;
		textOffset++;
		//if (textOffset < -j * (LET_WIDTH + SPACE)) {    // строка убежала
		if (textOffset > j * (LET_WIDTH + SPACE) + WIDTH) {    // строка убежала
			textOffset = 0;
			fullTextFlag = true;
		}
		fastLED.show();
