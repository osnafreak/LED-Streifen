...
void customModes()
{
	...
	case 0: 
		...
		fillString("FROHES NEUES JAHR!   FROHES NEUES JAHR 2026!", 2);	// getHEX(COLORS::mNavy)
		break;
	//case 1: fillString("HAPPY NEW YEAR!", 1);
		//break;
	//case 2: fillString("С НОВЫМ ГОДОМ!", 2);
		//break;
	//case 3: fillString("BONNE ANNEE!", 2);
		//break;
	//case 4: fillString("FELIZ ANO NUEVO!", 2);
		//break;
	...
	default:
		nextModeHandler();
		//thisMode = (thisMode + 1) % MODES_AMOUNT;
		break;
	}
}
...
int fadeBrightness = globalBrightness;
...
void nextModeHandler()
{
	...
	//gamemodeFlag = false;
	fastLED.clear();
	for (uint8_t x = WIDTH/2 - 1; x <= WIDTH/2 + 1; x++) {
		fastLED.set(x, HEIGHT/2, COLORS::mOrange);
		fastLED.show();
		_delay_ms(D_EFFECT_SPEED);
	}
}
...
void customRoutine()
{
	//if (!BTcontrol)
	//{
	if (modeCode == 0)	// Textmodus mit eigenem Timer
	{
		customModes();
	}
	else if (effectTimer.isReady())
	{
#if (OVERLAY_CLOCK == 1 && USE_CLOCK == 1)
		if (overlayAllowed()) {
			if (!loadingFlag && !gamemodeFlag && needUnwrap() && modeCode != 0)
				clockOverlayUnwrap(CLOCK_X, CLOCK_Y);
			if (loadingFlag) 
				loadFlag2 = true;
		}
#endif	// OVERLAY_CLOCK == 1 && USE_CLOCK == 1

		customModes();                // режимы крутятся, пиксели мутятся

#if (OVERLAY_CLOCK == 1 && USE_CLOCK == 1)
		if (overlayAllowed()) {
			if (!gamemodeFlag && modeCode != 0)
...
void timeSet(bool type, bool dir)
{    // type: 0-часы, 1-минуты, dir: 0-уменьшить, 1-увеличить
	if (type) {
		if (dir) hrs++;
		else hrs--;
	} else {
...
void btnsModeChange()
{
	...
	if (bt_set.clicked())
	{
		// Spiele sind aus, Variablen sparen
		//if (gamemodeFlag) 
			//gameDemo = !gameDemo;
		//if (gameDemo) {
			////gameSpeed = DEMO_GAME_SPEED;
