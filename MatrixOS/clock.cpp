...
#define MIN_COLOR COLORS::mWhite	// цвет минут
#define HOUR_COLOR COLORS::mWhite	// цвет часов
#define DOT_COLOR COLORS::mRed		// цвет точек
...
MColor overlayLEDs[75];
#elif (OVERLAY_CLOCK == 1 && CLOCK_ORIENT == 1)
MColor overlayLEDs[70];
#endif

//#if (USE_CLOCK == 1)
MColor clockLED[5] = {COLORS::mWhite, COLORS::mWhite, COLORS::mRed, COLORS::mWhite, COLORS::mWhite};

bool overlayAllowed()
{
	for (byte i = 0; i < listSize; i++)
		if (modeCode == overlayList[i]) 
			return true;
	return false;
}
...
void clockColor()
{
	if (COLOR_MODE == 0) {
		...
		for (byte i = 0; i < 5; i++)
			clockLED[i] = mHSV(clockHue + HUE_GAP * i, 255, 255);
		clockLED[2] = mHSV(clockHue + 128 + HUE_GAP * 1, 255, 255); // точки делаем другой цвет
	} else if (COLOR_MODE == 2) {
		clockLED[0] = mHSV(clockHue + HUE_GAP * 0, 255, 255);
		clockLED[1] = mHSV(clockHue + HUE_GAP * 0, 255, 255);
		clockLED[2] = mHSV(clockHue + 128 + HUE_GAP * 1, 255, 255); // точки делаем другой цвет
		clockLED[3] = mHSV(clockHue + HUE_GAP * 2, 255, 255);
		clockLED[4] = mHSV(clockHue + HUE_GAP * 2, 255, 255);
	}
}

// нарисовать часы
void drawClock(byte hrs, byte mins, bool dots, byte X, byte Y)
{
#if (CLOCK_ORIENT == 0)
	if (hrs > 9) drawDigit3x5(hrs / 10, X, Y, getHEX(clockLED[0]));
	drawDigit3x5(hrs % 10, X + 4, Y, getHEX(clockLED[1]));
	if (dots) {
		drawPixelXY(X + 7, Y + 1, getHEX(clockLED[2]));
		drawPixelXY(X + 7, Y + 3, getHEX(clockLED[2]));
	} else {
		if (modeCode == 1) {
	...
	drawDigit3x5(mins / 10, X + 8, Y, getHEX(clockLED[3]));
	drawDigit3x5(mins % 10, X + 12, Y, getHEX(clockLED[4]));
#else
	if (hrs > 9) drawDigit3x5(hrs / 10, X, Y + 5, getHEX(clockLED[0]));
	drawDigit3x5(hrs % 10, X + 4, Y + 5, getHEX(clockLED[1]));

	drawDigit3x5(mins / 10, X, Y, getHEX(clockLED[3]));
	drawDigit3x5(mins % 10, X + 4, Y, getHEX(clockLED[4]));
#endif
}
...
void clockOverlayWrap(byte posX, byte posY)
{
	...
	for (byte i = posX; i < posX + 15; i++) {
		for (byte j = posY; j < posY + 5; j++) {
			overlayLEDs[thisLED] = fastLED.getPixNumber(i, j);
			thisLED++;
		}
	}
	clockTicker();
...
void clockOverlayUnwrap(byte posX, byte posY)
{
	...
	for (byte i = posX; i < posX + 15; i++) {
		for (byte j = posY; j < posY + 5; j++) {
			fastLED.set(i, j, overlayLEDs[thisLED]);
			thisLED++;
		}
	}
}
...
{
	for (byte i = 0; i < 5; i++)
		clockLED[i] = COLORS::mBlack;
	clockLED[2] = COLORS::mWhite;
}
